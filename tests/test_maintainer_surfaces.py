...
def test_v23_status_doc_tracks_release_gates_and_next_blocker():
    ...
    assert (
        "Deep Claude live driver with SDK adapter and approval bridging" in status_doc
    )
    ...
    assert "Hybrid dense retrieval" in status_doc
    assert (
        "the Daytona self-hosted proof now passed in a credentialed environment"
        in status_doc
    )
    ...
    assert (
        "built-artifact release smoke path now proves installed-package `hive search`"
        in status_doc
    )
    assert "Status: v2.3.2 released" in status_doc
...
def test_v24_status_doc_tracks_scope_and_next_blocker():
    ...
    assert "| OpenClaw attach mode | Landed |" in status_doc
    assert "| Hermes companion integration | Landed |" in status_doc
    assert "| Console truth + attach visibility proof | Landed |" in status_doc
    assert (
...
def test_v23_acceptance_doc_tracks_scope_locked_remote_sandbox_truth():
    ...
    assert (
        "E2B pause/resume and downloaded artifact sync are deferred" in acceptance_doc
    )
    assert (
...
def test_ci_workflow_proves_local_safe_podman_path():
    ...
    assert any(
        step.get("name") == "Verify rootless Podman availability" for step in steps
    )
    assert any(
        step.get("name") == "Verify Hive sees Podman as local-safe" for step in steps
    )
    assert any(
...
def test_repo_relies_on_managed_claude_review_instead_of_repo_local_workflow():
    ...
    install_doc = (REPO_ROOT / "docs" / "INSTALL_CLAUDE_APP.md").read_text(
        encoding="utf-8"
    )
    maintaining_doc = (REPO_ROOT / "docs" / "MAINTAINING.md").read_text(
        encoding="utf-8"
    )
    ...
    assert (
        "does not ship a custom Claude GitHub Actions review workflow"
        in maintaining_doc
    )
    assert "An `eyes` reaction alone is not completion." in install_doc
    assert (
        "local Claude review is an acceptable primary or fallback path"
        in maintaining_doc
    )
