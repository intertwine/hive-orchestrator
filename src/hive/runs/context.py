...
def _write_claude_projection(
    ...
    *,
    ...
    source_claude = (
        source_claude_path.read_text(encoding="utf-8").strip()
        if source_claude_path.exists()
        else ""
    )
    skill_lines = [
        f"- `{skill['name']}` from `{skill['path']}`"
        for skill in selected_skills
        if skill.get("name") and skill.get("path")
    ]
    projection_lines = [
        "# CLAUDE",
        "",
        f"This projection was generated by Hive for run `{run_id}`.",
        "Use the run brief and the canonical Hive files as the source of truth.",
        "",
        "## Canonical Inputs",
        "",
    ...
    if skill_lines:
        projection_lines.extend(["", "## Selected Skills", "", *skill_lines])
    if source_claude:
        projection_lines.extend(["", "## Workspace CLAUDE.md", "", source_claude])
    else:
        projection_lines.extend(
            [
                ...
                "## Workspace Guidance",
                "",
                "Follow the run brief and the canonical Hive workspace files.",
            ]
        )
    claude_projection_path.write_text(
...
def compile_run_context(
    ...
    *,
    ...
    claude_projection_path = None
    claude_skill_outputs: list[str] = []
    if driver in {"claude", "claude-code"}:
        claude_projection_path, claude_skill_outputs = _write_claude_projection(
            run_directory,
            run_id=run_id,
            source_claude_path=root / "CLAUDE.md",
            run_brief_path=run_brief_path,
            selected_skills=selected_skills,
        )
        outputs.append(claude_projection_path)
        outputs.extend(claude_skill_outputs)
    ...
    return {
        ...
        "claude_projection_path": (
            str(run_directory / claude_projection_path) if claude_projection_path else None
        ),
        "claude_skill_outputs": [str(run_directory / Path(path)) for path in claude_skill_outputs],
