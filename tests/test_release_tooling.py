...
def test_wheel_force_include_uses_repo_docs_as_the_only_markdown_source():
    ...
    assert (
        force_include["docs/V2_5_RELEASE_WALKTHROUGH.md"]
        == "src/hive/resources/docs/V2_5_RELEASE_WALKTHROUGH.md"
    )
...
def test_packaged_search_indexes_v25_status_and_planning_docs():
    """Installed API search should expose the active v2.5 ledger, walkthrough, and planning corpus."""
    search_source = (Path(__file__).resolve().parents[1] / "src" / "hive" / "search.py").read_text(
        encoding="utf-8"
    )

    assert '"docs/V2_5_STATUS.md"' in search_source
    assert '"docs/V2_5_RELEASE_WALKTHROUGH.md"' in search_source
