...
- Local Claude fallback review was not reliably usable through the default non-interactive exec path. In this session `claude -p "/review 201"` appeared to hang until we switched to a TTY-backed invocation and, eventually, to a smaller scoped diff review prompt.
- Even the TTY-backed local Claude fallback remained brittle when using the built-in `/review <pr>` flow. On PR #202, the reliable path was a plain-language prompt to review the current `origin/main...HEAD` diff, which suggests the maintainer helper should be diff-first rather than PR-command-first.
...
- The default frontend unit script can produce a false sense of coverage during focused work. `pnpm --dir frontend/console run test -- src/test/consoleAccessibility.test.tsx` still respects the script-level `--exclude=src/test/consoleAccessibility.test.tsx`, so an explicit request to run the accessibility spec silently skips the very file the maintainer asked for.
...
- A post-merge canonical-state sync commit that only touched task/projection docs still sent `main` red because a frontend smoke test had a latent async race. When that happens, maintainers have to manually infer that the failure is unrelated to the changed surface because Hive and CI do not explain likely attribution.
...
- Add a first-class maintainer helper for local Claude review that chooses the reliable invocation mode automatically, streams progress, and supports scoped diff review without ad hoc shell plumbing.
- Make the local review helper prefer “review the current diff against <base>” prompts over `/review <pr>` when the latter is slow or silent, and surface that fallback choice explicitly so maintainers know which review mode actually ran.
